...
static int sunxi_ahub_dai_startup(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		snd_soc_dai_set_dma_data(dai, substream, &ahub_info->playback_dma_param);
	} else {
		snd_soc_dai_set_dma_data(dai, substream, &ahub_info->capture_dma_param);
	}
				  ...
				  unsigned int freq_in, unsigned int freq_out)
{
	...
	if (freq_in > 24576000) {
		...
		if (clk_set_rate(clk_info->clk_pllx4, freq_in)) {
				    ...
				    struct snd_soc_dai *dai)
{
	...
	if (ahub_info->dts_info.dai_type == SUNXI_DAI_HDMI_TYPE) {
		ahub_info->hdmi_fmt = snd_sunxi_hdmi_get_fmt();
	...
	case SNDRV_PCM_FORMAT_S16_LE:
		...
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			if (ahub_info->dts_info.dai_type == SUNXI_DAI_HDMI_TYPE) {
				if (ahub_info->hdmi_fmt > HDMI_FMT_PCM) {
	...
	if (ahub_info->dts_info.dai_type == SUNXI_DAI_HDMI_TYPE) {
		ret = snd_sunxi_hdmi_hw_params(params, ahub_info->hdmi_fmt);
		if (ret) {
...
static int sunxi_ahub_dai_hw_free(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	...
	sunxi_ahub_dam_ctrl(false, ahub_info->dts_info.apb_num, ahub_info->dts_info.tdm_num, 0);
...
static int sunxi_ahub_probe(struct snd_soc_component *component)
{
	...
	if (ahub_info->dts_info.dai_type == SUNXI_DAI_HDMI_TYPE) {
		...
		ahub_info->hdmi_fmt = HDMI_FMT_NULL;
	}
					      ...
					      struct sunxi_ahub_dts_info *dts_info)
{
	...
	ret = snd_sunxi_hdmi_get_dai_type(np, &dts_info->dai_type);
	if (ret)
		dts_info->dai_type = SUNXI_DAI_I2S_TYPE;

	if (dts_info->dai_type == SUNXI_DAI_HDMI_TYPE)
		SND_LOG_DEBUG(HLOG, "dai_type     : HDMI\n");
	else
		SND_LOG_DEBUG(HLOG, "dai_type     : I2S\n");
...
static void snd_soc_sunxi_dma_params_init(struct sunxi_ahub_info *ahub_info)
{
	...
	ahub_info->playback_dma_param.src_maxburst = 4;
	ahub_info->playback_dma_param.dst_maxburst = 4;
	ahub_info->playback_dma_param.dma_addr = res->start
						 + SUNXI_AHUB_APBIF_TXFIFO(dts_info->apb_num);
	ahub_info->playback_dma_param.cma_kbytes = dts_info->playback_cma;
	ahub_info->playback_dma_param.fifo_size = dts_info->playback_fifo_size;

	ahub_info->capture_dma_param.src_maxburst = 4;
	ahub_info->capture_dma_param.dst_maxburst = 4;
	ahub_info->capture_dma_param.dma_addr = res->start
						+ SUNXI_AHUB_APBIF_RXFIFO(dts_info->apb_num);
	ahub_info->capture_dma_param.cma_kbytes = dts_info->capture_cma;
	ahub_info->capture_dma_param.fifo_size = dts_info->capture_fifo_size;
};
...
static int sunxi_ahub_dev_probe(struct platform_device *pdev)
{
	...
	if (dts_info->dai_type == SUNXI_DAI_HDMI_TYPE)
		ret = snd_soc_sunxi_dma_platform_register(&pdev->dev, 1);
	else
